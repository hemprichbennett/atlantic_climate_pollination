...
file = "./data/processed_data/plants/03_thin_rec.csv"
...
for (a in 1:length(sp_names)){
  ...
  if (nrow(presences) < n_min){ ##Will not analyze species with less than 15 occurences
  ...
  raster_files <- list.files(paste0("./data/processed_data/plants/", sp.n, "/Pres_env_crop"), full.names = T, 'tif$|bil$')
  head(raster_files)
  ...
  ## RCP45
  raster_files2 <- list.files(paste0("./data/processed_data/plants/", sp.n, "/Fut_env_crop/", RCP1), full.names = T, 'tif$|bil$')
  head(raster_files2)
  ...
  ####RCP85
  raster_files3 <- list.files(paste0("./data/processed_data/plants/", sp.n, "/Fut_env_crop/", RCP2), full.names = T, 'tif$|bil$')
  head(raster_files3)
  ...
  target_dir = paste(paste0("./results/plants", sp.n, "/results/", sep=""))
  ...
  ## pseudoausencia = n*10
  sp.data <- read.csv(paste0("./data/processed_data/plants/", sp.n,"/pres_pseudoabs2.csv"), header=TRUE, sep=',')

  #colocar aqui embaixo  outra planilha depois de rodar pro outro modelo
  ##pseudoausencia = length(sp$species)
  sp.data2 <- read.csv(paste0("./data/processed_data/plants/", sp.n,"/pres_pseudoabs.csv"), header=TRUE, sep=',')
  ...
  for(i in 1:k){
    ...
    predtrain3[[i]] <- raster::extract(predictors, train3[[i]])
  ...
  for(i in 1:k){
    ...
    evgm[[i]] <- dismo::evaluate(prestest3[[i]], abstest3[[i]], gm[[i]], predictors)
  ...
  if(all(TSS < tss.lim)){
    ...
    save.image("./results/plants/my_analysis.rData")
  ...
  algo.cur <- algo.cur[lapply(algo.cur, length) > 0] ##ignores NULL values
  ...
  if(length(wrf.sem.os.nulos) == 0){
    ...
    future_variable.rf.mean.w <- raster::weighted.mean(st3, wrf.sem.os.nulos)
  ...
  if(!is.null(ens.cur)){
    ...
    writeRaster(ens2.future_variable, file = paste(target_dir, '/Fut.HGemES.', RCP1, '.cont_', sp.n, '.asc', sep=""),overwrite=TRUE)
    writeRaster(ens2.future_variable2, file = paste(target_dir, '/Fut.HGemES.', RCP2, '.cont_', sp.n, '.asc', sep=""),overwrite=TRUE)

  }
  if(!is.null(ens.cur.bin)){
    writeRaster(ens.cur.bin, file = paste(target_dir, '/CUR.bin_', sp.n, '.asc', sep=""),overwrite=TRUE)
    writeRaster(ens.future_variable.bin, file = paste(target_dir, '/Fut.HGemES.', RCP1, '.bin_', sp.n, '.asc', sep=""),overwrite=TRUE)
    writeRaster(ens.future_variable2.bin, file = paste(target_dir, '/Fut.HGemES.', RCP2,'.bin_', sp.n, '.asc', sep=""),overwrite=TRUE)
  }
  ...
  if(!is.null(ens.cur.sd.w)){
    ...
    writeRaster(ens2.fut.sd.w, file = paste(target_dir, 'ens.Fut.HGemES.', RCP1, '.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
    writeRaster(ens2.fut.sd.w2, file = paste(target_dir, 'ens.Fut.HGemES.', RCP2, '.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
  ...
  if(!is.null(future_variable.gm.sd.w)){
    writeRaster(future_variable.gm.sd.w, file = paste(target_dir, 'Fut.HGemES.', RCP1, '.gm.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
  }

  if(!is.null(future_variable2.gm.sd.w)){
    writeRaster(future_variable2.gm.sd.w, file = paste(target_dir, 'Fut.HGemES.', RCP2, '.gm.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
  }
  ...
  if(!is.null(future_variable.rf.sd.w)){
    writeRaster(future_variable.rf.sd.w, file = paste(target_dir, '/Fut.HGemES.', RCP1, '.rf.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
  }

  if(!is.null(future_variable2.rf.sd.w)){
    writeRaster(future_variable2.rf.sd.w, file = paste(target_dir, '/Fut.HGemES.', RCP2, '.rf.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
  }
  ...
  if(!is.null(future_variable.mx.sd.w)){
    writeRaster(future_variable.mx.sd.w, file = paste(target_dir, '/Fut.HGemES.', RCP1, '.mx.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
  }

  if(!is.null(future_variable2.mx.sd.w)){
    writeRaster(future_variable2.mx.sd.w, file = paste(target_dir, '/Fut.HGemES.', RCP2, '.mx.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
  }
  ...
  if(!is.null(future_variable.sv.sd.w)){
    writeRaster(future_variable.sv.sd.w, file = paste(target_dir, '/Fut.HGemES.', RCP1, '.sv.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
  }

  if(!is.null(future_variable2.sv.sd.w)){
    writeRaster(future_variable2.sv.sd.w, file = paste(target_dir, '/Fut.HGemES.', RCP2, '.sv.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
  }

  rm(ens.cur, ens2.future_variable, ens2.future_variable2, ens.cur.bin, ens.future_variable.bin, ens.future_variable2.bin,ens.cur.sd.w, ens2.fut.sd.w, ens2.fut.sd.w2, cur.gm.sd.w, cur.rf.sd.w, cur.mx.sd.w, cur.sv.sd.w, future_variable.gm.sd.w, future_variable2.gm.sd.w, future_variable.rf.sd.w, future_variable2.rf.sd.w, future_variable.mx.sd.w, future_variable.sv.sd.w, future_variable2.sv.sd.w)
