...
to_crop <- raster('./data/processed_data/env_cropped/present/plants/_wc2.1_5m_bio_2.tif')
...
writeRaster(envi_fut.mask, filename='./data/processed_data/env_sel/future/plants/RCP85/', 
            format="GTiff", bylayer=TRUE, suffix="names", overwrite=TRUE)

# writeRaster(envi_fut.mask, filename='./data/processed_data/env_sel/future/plants/RCP45/', 
#            format="GTiff", bylayer=TRUE, suffix="names", overwrite=TRUE)
